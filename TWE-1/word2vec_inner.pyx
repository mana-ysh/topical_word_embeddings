...
ctypedef void (*fast_sentence_topic_ptr) (
    const np.uint32_t *word_point, const np.uint8_t *word_code, const int codelen,
    REAL_t *syn0, REAL_t *syn1, REAL_t *syn0_topic, const int size,
    np.uint32_t topic_index, const REAL_t alpha, REAL_t *work) nogil
...
cdef fast_sentence_topic_ptr fast_sentence_topic
...
cdef void fast_sentence0_topic(
    const np.uint32_t *word_point, const np.uint8_t *word_code, const int codelen,
    REAL_t *syn0, REAL_t *syn1, REAL_t *syn0_topic, const int size,
     np.uint32_t topic_index, const REAL_t alpha, REAL_t *work) nogil:
    ...
    for b in range(codelen):
        row2 = word_point[b] * size
        f = <REAL_t>dsdot(&size, &syn0_topic[row1], &ONE, &syn1[row2], &ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (1 - word_code[b] - f) * alpha
...
cdef void fast_sentence1_topic(
    const np.uint32_t *word_point, const np.uint8_t *word_code, const int codelen,
    REAL_t *syn0, REAL_t *syn1, REAL_t *syn0_topic, const int size,
     np.uint32_t topic_index, const REAL_t alpha, REAL_t *work) nogil:
...
cdef void fast_sentence2_topic(
    const np.uint32_t *word_point, const np.uint8_t *word_code, const int codelen,
    REAL_t *syn0, REAL_t *syn1, REAL_t *syn0_topic, const int size,
    const np.uint32_t topic_index, const REAL_t alpha, REAL_t *work) nogil:
    ...
    for b in range(codelen):
        ...
        f = <REAL_t>0.0
        for a in range(size):
            f += syn0_topic[row1 + a] * syn1[row2 + a]
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (1 - word_code[b] - f) * alpha
...
#By Largellymfs
def train_sentence_topic(model, sentence, alpha, _work):
    cdef int hs = model.hs
    cdef int negative = model.negative
    ...
    for i in range(sentence_len):
        ...
        if word is None:
            ...
            if hs:
                ...
                codelens[i] = 1
            result += 1
    cdef np.uint32_t *topic[MAX_SENTENCE_LEN]
    #topic = [item[1] for item in sentence]
    for i in range(sentence_len):
        topic[i] = sentence[i][1]
    # release GIL & train on the sentence
    with nogil:
        for i in range(sentence_len):
            if codelens[i] == 0:
...
def init():
    ...
    if (abs(d_res - expected) < 0.0001):
        ...
        fast_sentence_topic = fast_sentence0_topic
        return 0  # double
        ...
        fast_sentence_topic = fast_sentence1_topic
        return 1  # float
    else:
        ...
        fast_sentence_topic = fast_sentence2_topic
        return 2
